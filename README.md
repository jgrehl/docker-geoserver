...
```
VERSION=2.25.2
IMAGE_VERSION=9.0-jdk11-temurin-focal
...
```
docker build --build-arg IMAGE_VERSION=9.0-jdk11-temurin-focal --build-arg JAVA_HOME=/usr/local/openjdk-11/bin/java --build-arg GS_VERSION=2.25.2 -t kartoza/geoserver:2.25.2 .
```
...
The current build uses the base image `tomcat:9.0-jdk11-temurin-focal` because of the dependency on
